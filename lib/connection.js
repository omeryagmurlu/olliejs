...
module.exports = function(uuid, opts) {
	return new Connection(uuid, opts);
}

var Connection = function Connection(uuid, opts){
	opts = opts || {};
	this.uuid = uuid;
	this.peripheral;
...
Connection.prototype.init = function(callback) { //need no callback here, but if you love callback chains, you can pass it
	...
	this.noble.on("discover", function(peripheral) {
		if (peripheral.id === this.uuid) {
			...
			this.emit('connect', peripheral); // this defeats the purpose of callback
